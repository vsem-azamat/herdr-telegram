...
With explicit owner approval, the contract was implemented in the personal [`vsem-azamat/herdr` fork](https://github.com/vsem-azamat/herdr), independently reviewed, rebased onto upstream `02fe7d76`, and squash-merged through [fork PR #1](https://github.com/vsem-azamat/herdr/pull/1) at [`a8758ff3`](https://github.com/vsem-azamat/herdr/commit/a8758ff38e17e7b84c097f7b421fa15c9510f02e). The fork advertises `agent_prompt_expected_session`, compares the complete native identity before input is queued, returns `agent_session_mismatch`, and pins waits to the native session. Its full `just check` passed, including deterministic serialized replacement-order and no-input-on-rejection tests. A side-by-side installed binary also advertised the capability in a disposable socket smoke test; the system Herdr binary was not replaced.
...
The redacted disposable live fixture is now tracked in [`herdr-expected-session-live-probe.md`](herdr-expected-session-live-probe.md). A later internal bridge adapter must require the capability; the SDK models the wire field but does not itself own bridge startup policy. The `PromptWait` turn-correlation limitation must remain documented and separately resolved before completion evidence is used to release queued turns.

## Upstream publication status

With explicit owner approval, the contract gap was published as [Herdr Discussion #2016: Allow `agent.prompt` to check the expected session ID](https://github.com/herdrdev/herdr/discussions/2016). It remains a direction request rather than an accepted upstream contract. The authorized personal fork merge is documented above. Do not create another issue or PR against `herdrdev/herdr` without separate explicit owner approval and maintainer alignment.
